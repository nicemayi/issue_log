...
<template>
    <div class="issue-detail panel-body">
        <div>
            <h5>
                This is a
                <b style="color: red;">{{issue_detail.issue_type}}</b> issue from client
                <b style="color: red;">{{issue_detail.client_id}}</b>, opened by
                <b style="color: red;">{{issue_detail.create_by}}</b> at
                <b style="color: red;">{{issue_detail.create_time}}</b>
            </h5>
        </div>
        <div>
            <h5>
        ...
        <div v-else>
            <h5>
                <b>2. No involved patient samples.</b>
            </h5>
        </div>
        <div>
                ...
                <ul>
                    <li v-for="each_comment in issue_detail.comments">
                        <h5 v-if="each_comment.comment_type === 'DEPARTMENT_CLOSE'">
                            <span class="label label-primary" style="margin-right: 0.5rem;">
                        ...
                        <h5 v-if="each_comment.comment_type === 'CLOSE_ISSUE'">
                            <span class="label label-success" style="margin-right: 0.5rem;">
        ...
        <div v-if="issue_detail.is_closed == '0'">
            <div>
                ...
                <div class="form-group">
                    <textarea class="form-control" rows="5" v-model="action_content">
                    </textarea>
                </div>
            </div>
            <div>
                <h5>
                    <b>Add Department / Send notifications</b>
                </h5>
                <div>
                    <div>
                        <el-form ref="form">
                            <el-form-item width="50%">
                                <el-select v-model="add_department_or_person" multiple placeholder="Select" style="width: 30%;">
                                    <el-option-group v-for="group in groups" :label="group.label">
                                        <el-option v-for="person in group.persons" :label="person.label" :value="person.value">
                                        </el-option>
                                    </el-option-group>
                                </el-select>
        ...
        <div v-if="issue_detail.departments.length + issue_detail.notifications.length">
            ...
            <div v-if="issue_detail.departments.length > 0">
                ...
                <span v-for="(each_department, index) in issue_detail.departments">
                    <button :disabled="canBeRemoved" data-toggle="modal" :data-target="'#' + 'department-' + index" class="btn btn-info" style="margin-right: 1%; margin-bottom: 1%;">
                        <b style="color: black;">{{each_department}}</b>
                    </button>
            ...
            <div v-if="issue_detail.notifications.length > 0">
                ...
                <span v-for="(each_user, index) in issue_detail.notifications">
                    <button :disabled="canBeRemoved" data-toggle="modal" :data-target="'#' + 'notification-' + index" class="btn btn-info" style="margin-right: 1%; margin-bottom: 1%;">
                        <b style="color: black;">{{each_user}}</b>
                    </button>
